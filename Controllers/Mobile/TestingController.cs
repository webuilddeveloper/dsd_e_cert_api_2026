...
namespace mobile_api.Controllers
{
    ...
    public class TestingController : Controller
    {
        ...
        //    try
        //    {
        //        List<TestingModel> docs = new List<TestingModel>();

        //        docs = await $"http://119.13.28.171:8888/globe/testing/{value.keySearch}".HttpGet<List<TestingModel>>();
        //        if (docs == null) docs = new List<TestingModel>();

        //        var colCategory = new Database().MongoClient<Category>("testingCategory");
        ...
        //                                            .Project(p => p.testingCategory).ToList();
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Testing>( "testing");
                var filter = (Builders<Testing>.Filter.Eq("status", "A") );

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Testing>.Filter.Eq("code", value.code); }
                if (!string.IsNullOrEmpty(value.keySearch)) { filter = filter & Builders<Testing>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")); }
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<Testing>.Filter.Eq("category", value.category); }
                if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<Testing>.Filter.Eq("status", value.status); }
                if (!string.IsNullOrEmpty(value.description)) { filter = filter & Builders<Testing>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Testing>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<Testing>.Filter.Eq("isHighlight", value.isHighlight); }

                var ds = value.startDate.toDateFromString().toBetweenDate();
                var de = value.endDate.toDateFromString().toBetweenDate();
                if (value.startDate != "Invalid date" && value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate) && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Testing>.Filter.Gt("docDate", ds.start) & Builders<Testing>.Filter.Lt("docDate", de.end); }
                else if (value.startDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<Testing>.Filter.Gt("docDate", ds.start) & Builders<Testing>.Filter.Lt("docDate", ds.end); }
                else if (value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Testing>.Filter.Gt("docDate", de.start) & Builders<Testing>.Filter.Lt("docDate", de.end); }
                //filter = filter & (Builders<BsonDocument>.Filter.Eq(x => x.B, "4") | Builders<User>.Filter.Eq(x => x.B, "5"));

                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new Testing { 
                    code = c.code,
                    isActive = c.isActive,
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient( "testing");
...

                    {
                        ...
                        var colUpdateReadContent = new Database().MongoClient("testingReadContent");
                ...
                docs.ForEach(c => {
                    ...
                    {
                        var colTR = new Database().MongoClient<Testing>("testingRegister");
                        var filterTR = Builders<Testing>.Filter.Eq("createBy", value.username) & Builders<Testing>.Filter.Eq("reference", c.code);
        ...
        // POST /read
        [HttpPost("mark/read")]
        public ActionResult<Response> MarkRead([FromBody] Testing value)
        {
            try
            {
                if (value.year == 0) {
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>( "testingGallery");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Comment>("testingComment");
        ...
        public ActionResult<Response> CommentCreate([FromBody] Comment value)
        {
            ...
            try
            {
                ...
                var col = new Database().MongoClient("testingComment");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Category>( "testingCategory");
        ...
        public ActionResult<Response> RegisterCreate([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "testingRegister");
        ...
        public ActionResult<Response> RegisterRead([FromBody] Criteria value)
        {
            try
            {
                value.statisticsCreate("testingRegister");
                var col = new Database().MongoClient<Comment>( "testingRegister");

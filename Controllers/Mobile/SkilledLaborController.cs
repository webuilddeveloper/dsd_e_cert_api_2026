    ...
    public class SkilledLaborController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<SkilledLabor>.Filter.Eq("category", value.category); }
                if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<SkilledLabor>.Filter.Eq("status", value.status); }
                if (!string.IsNullOrEmpty(value.description)) { filter = filter & Builders<SkilledLabor>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<SkilledLabor>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<SkilledLabor>.Filter.Eq("isHighlight", value.isHighlight); }
                ...
                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new SkilledLabor
                {
                    code = c.code,
                    isActive = c.isActive,
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient("skilledLabor");
                ...
                docs.ForEach(c => {
                    ...
                    {
                        var colTR = new Database().MongoClient<SkilledLabor>("skilledLaborRegister");
                        var filterTR = Builders<SkilledLabor>.Filter.Eq("createBy", value.username) & Builders<SkilledLabor>.Filter.Eq("reference", c.code);
        ...
        // POST /read
        [HttpPost("mark/read")]
        public ActionResult<Response> MarkRead([FromBody] SkilledLabor value)
        {
            try
            {
                if (value.year == 0)
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("skilledLaborGallery");
        ...
        public ActionResult<Response> RegisterCreate([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("skilledLaborRegister");

    ...
    public class NewsController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var colCategory = new Database().MongoClient<Category>("newsCategory");
                ...
                var docs = col.Aggregate().Match(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new News {
                code = c.code,
                title = c.title,
                ...
                docs.ForEach(c =>
                {
                    ...
                    foreach (var o in docRegister)
                    {
                        ...
                        else
                        {
                            c.userList.Add(new Register1
                            {
                                //imageUrl = c.imageUrlCreateBy,
                                firstName = c.createBy,
                                lastName = ""
                            });
                        }
                    }
                    foreach (var o in docsCategory)
                    {
                        if (c.category == o.code)
                        {
                            c.categoryList.Add(new Category
                            {
                                title = o.title,
                            });
                            break;
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.updateDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.code, c.title,c.titleEN, c.language, c.imageUrl, c.createBy, c.createDate, c.isActive }).ToList();
                ...
                try
                {
                    ...
                    value.statisticsCreate("newsCategory");  
                }
                catch { }
                //END : Statistic

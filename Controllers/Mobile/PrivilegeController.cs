    ...
    public class PrivilegeController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                if (value.isPublic) { filter = filter & Builders<Privilege>.Filter.Eq("isPublic", value.isPublic); }
        ...
        public ActionResult<Response> SuggestionCategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.code, c.title,c.titleEN, c.imageUrl, c.language, c.createBy, c.createDate, c.isActive }).ToList();

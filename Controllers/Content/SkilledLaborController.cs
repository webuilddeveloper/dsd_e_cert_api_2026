...
{
    [Route("[controller]")]
    public class SkilledLaborController : Controller
    {
        public SkilledLaborController() { }

        #region main

        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] SkilledLabor value)
        {

            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient("skilledLabor");
                ...
                {
                    Task.Run(() => { value.organization.createQrganizationAuto(value.code, "skilledLabor"); });
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = (Builders<SkilledLabor>.Filter.Ne("status", "D") & value.filterOrganization<SkilledLabor>());

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<SkilledLabor>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<SkilledLabor>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));

                    if (value.permission != "all")
                        filter &= value.permission.filterPermission<SkilledLabor>("category");
                }
                else
                {

                    if (!string.IsNullOrEmpty(value.category))
                        filter &= Builders<SkilledLabor>.Filter.Eq("category", value.category);
                    else
                        if (value.permission != "all")
                        filter &= value.permission.filterPermission<SkilledLabor>("category");


                    if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<SkilledLabor>.Filter.Regex("code", value.code); }
                    if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<SkilledLabor>.Filter.Eq("status", value.status); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter = filter & Builders<SkilledLabor>.Filter.Eq("createBy", value.createBy); }
                    if (!string.IsNullOrEmpty(value.title)) { filter = filter & Builders<SkilledLabor>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<SkilledLabor>.Filter.Regex("language", value.language); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<SkilledLabor>.Filter.Eq("sequence", sequence); }

                    var ds = value.startDate.toDateFromString().toBetweenDate();
                    var de = value.endDate.toDateFromString().toBetweenDate();
                    if (value.startDate != "Invalid date" && value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate) && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<SkilledLabor>.Filter.Gt("docDate", ds.start) & Builders<SkilledLabor>.Filter.Lt("docDate", de.end); }
                    else if (value.startDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<SkilledLabor>.Filter.Gt("docDate", ds.start) & Builders<SkilledLabor>.Filter.Lt("docDate", ds.end); }
                    else if (value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<SkilledLabor>.Filter.Gt("docDate", de.start) & Builders<SkilledLabor>.Filter.Lt("docDate", de.end); }
                }
                ...
                List<SkilledLabor> docs = col.Aggregate().Match(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                                     .Lookup("skilledLaborCategory", "category", "code", "categoryList")
                                     .As<SkilledLabor>()
                                     .ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] SkilledLabor value)
        {
            var doc = new BsonDocument();
            try
            {
                ...
                if (value.generation > 0) { doc["generation"] = value.generation; }
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] SkilledLabor value)
        {
            try
            {
                var col = new Database().MongoClient("skilledLabor");
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("skilledLaborGallery");
        ...
        public ActionResult<Response> CommentCreate([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("skilledLaborComment");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Comment>("skilledLaborComment");
                ...
                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.updateDate).Skip(value.skip).Limit(value.limit).Project(c => new { c.description, c.createBy, c.createDate, c.imageUrlCreateBy }).ToList();
        ...
        public ActionResult<Response> CategoryCreate([FromBody] Category value)
        {
            ...
            try
            {
                var col = new Database().MongoClient("skilledLaborCategory");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Category>("skilledLaborCategory");
        ...
        public ActionResult<Response> CategoryUpdate([FromBody] Category value)
        {
            ...
            try
            {
                ...
                if (!value.isActive)
                {
                    var collectionContent = new Database().MongoClient("skilledLabor");
                ...
                if (!value.isActive)
                {
                    ...
                    var updatePermission = Builders<BsonDocument>.Update.Set("skilledLaborPage", false).Set("isActive", false);
        ...
        public ActionResult<Response> RegisterRead([FromBody] Criteria value)
        {
            try
            {
                value.statisticsCreate("skilledLaborRegister");
                var col = new Database().MongoClient<Comment>("skilledLaborRegister");

                var filter = Builders<Comment>.Filter.Eq(x => x.isActive, true);
                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Comment>.Filter.Regex("reference", value.code); }
                //filter = filter & (Builders<BsonDocument>.Filter.Eq(x => x.B, "4") | Builders<User>.Filter.Eq(x => x.B, "5"));

                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.updateTime).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.reference, c.createBy, c.createDate, c.imageUrlCreateBy, c.type, c.idcard }).ToList();
        ...
        // POST /create
        [HttpPost("reorder")]
        public ActionResult<Response> ReOrder([FromBody] SkilledLabor value)
        {

            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient<BsonDocument>("skilledLabor");

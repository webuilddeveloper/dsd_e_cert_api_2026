...
namespace cms_api.Controllers
{
    ...
    public class TrainingController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] Training value)
        {

            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient("training");
                ...
                {
                    Task.Run(() => { value.organization.createQrganizationAuto(value.code, "training"); });
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = (Builders<Training>.Filter.Ne("status", "D") & value.filterOrganization<Training>());

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Training>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Training>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));

                    if (value.permission != "all")
                        filter &= value.permission.filterPermission<Training>("category");
                }
                else
                {

                    if (!string.IsNullOrEmpty(value.category))
                        filter &= Builders<Training>.Filter.Eq("category", value.category);
                    else
                        if (value.permission != "all")
                        filter &= value.permission.filterPermission<Training>("category");


                    if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Training>.Filter.Regex("code", value.code); }
                    if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<Training>.Filter.Eq("status", value.status); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter = filter & Builders<Training>.Filter.Eq("createBy", value.createBy); }
                    if (!string.IsNullOrEmpty(value.title)) { filter = filter & Builders<Training>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Training>.Filter.Regex("language", value.language); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<Training>.Filter.Eq("sequence", sequence); }

                    var ds = value.startDate.toDateFromString().toBetweenDate();
                    var de = value.endDate.toDateFromString().toBetweenDate();
                    if (value.startDate != "Invalid date" && value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate) && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Training>.Filter.Gt("docDate", ds.start) & Builders<Training>.Filter.Lt("docDate", de.end); }
                    else if (value.startDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<Training>.Filter.Gt("docDate", ds.start) & Builders<Training>.Filter.Lt("docDate", ds.end); }
                ...
                List<Training> docs = col.Aggregate().Match(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                                     .Lookup("trainingCategory", "category", "code", "categoryList")
                                     .As<Training>()
                                     .ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] Training value)
        {
            var doc = new BsonDocument();
            try
            {
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] Training value)
        {
            try
            {
                var col = new Database().MongoClient( "training");
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "trainingGallery");
        ...
        // POST /create
        [HttpPost("reorder")]
        public ActionResult<Response> ReOrder([FromBody] Training value)
        {

            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient<BsonDocument>("training");

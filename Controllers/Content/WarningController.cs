...
namespace cms_api.Controllers
{
    ...
    public class WarningController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] Warning value)
        {
            
            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient("warning");
                ...
                {
                    Task.Run(() => { value.organization.createQrganizationAuto(value.code, "warning"); });
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var col = new Database().MongoClient<Warning>( "warning");
                var filter = (Builders<Warning>.Filter.Ne("status", "D") & value.filterOrganization<Warning>());

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Warning>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Warning>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));

                    if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Warning>("category"));
                }
                else
                {
                    if (!string.IsNullOrEmpty(value.category))
                        filter &= Builders<Warning>.Filter.Eq("category", value.category);
                    else
                        if (value.permission != "all")
                        filter &= (value.permission.filterPermission<Warning>("category"));

                    if (!string.IsNullOrEmpty(value.code)) { filter &= Builders<Warning>.Filter.Eq("code", value.code); }
                    if (!string.IsNullOrEmpty(value.status)) { filter &= Builders<Warning>.Filter.Eq("status", value.status); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter &= Builders<Warning>.Filter.Eq("createBy", value.createBy); }
                    if (!string.IsNullOrEmpty(value.title)) { filter &= Builders<Warning>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.description)) { filter &= Builders<Warning>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                    if (!string.IsNullOrEmpty(value.language)) { filter &= Builders<Warning>.Filter.Regex("language", value.language); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<Warning>.Filter.Eq("sequence", sequence); }

                    var ds = value.startDate.toDateFromString().toBetweenDate();
                    var de = value.endDate.toDateFromString().toBetweenDate();
                    if (value.startDate != "Invalid date" && value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate) && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Warning>.Filter.Gt("docDate", ds.start) & Builders<Warning>.Filter.Lt("docDate", de.end); }
                    else if (value.startDate != "Invalid date" && !string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<Warning>.Filter.Gt("docDate", ds.start) & Builders<Warning>.Filter.Lt("docDate", ds.end); }
                    else if (value.endDate != "Invalid date" && !string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<Warning>.Filter.Gt("docDate", de.start) & Builders<Warning>.Filter.Lt("docDate", de.end); }
                ...
                List<Warning> docs = col.Aggregate().Match(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                                      .Lookup("warningCategory", "category", "code", "categoryList")
                                      .As<Warning>()
                                      .ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] Warning value)
        {
            var doc = new BsonDocument();
            try
            {
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] Warning value)
        {
            try
            {
                var col = new Database().MongoClient( "warning");
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "warningGallery");
        ...
        public ActionResult<Response> CommentCreate([FromBody] Comment value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "warningComment");
        ...
        public ActionResult<Response> CategoryCreate([FromBody] Category value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "warningCategory");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Category>( "warningCategory");
        ...
        public ActionResult<Response> CategoryUpdate([FromBody] Category value)
        {
            ...
            try
            {
                ...
                if (!value.isActive)
                {
                    var collectionContent = new Database().MongoClient("warning");
                ...
                if (!value.isActive)
                {
                    ...
                    var updatePermission = Builders<BsonDocument>.Update.Set("warningPage", false).Set("isActive", false);
        ...
        public ActionResult<Response> ReOrder([FromBody] EventCalendar value)
        {
            ...
            try
            {
                var col = new Database().MongoClient<BsonDocument>("warning");

    ...
    public class KnowledgeController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                {
                    filter = filter & Builders<Knowledge>.Filter.Regex("title", value.keySearch ) | (filter & Builders<Knowledge>.Filter.Regex("description", value.keySearch)) | (filter & Builders<Knowledge>.Filter.Regex("titleEN", value.keySearch));
        ...
        public ActionResult<Response> Update([FromBody] Knowledge value)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.authorEN)) { doc["authorEN"] = value.authorEN; }
                ...
                if (!string.IsNullOrEmpty(value.publisherEN)) { doc["publisherEN"] = value.publisherEN; }
                if (!string.IsNullOrEmpty(value.publishDate)) { doc["publishDate"] = value.publishDate; }
                   if (!string.IsNullOrEmpty(value.textButton)) { doc["textButtonEN"] = value.textButtonEN; }
        ...
        public ActionResult<Response> knowledgeCategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                var docs = col.Find(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.code, c.title,c.titleEN, c.language, c.imageUrl, c.createBy, c.createDate, c.isActive, c.sequence, c.updateBy, c.updateDate }).ToList();
